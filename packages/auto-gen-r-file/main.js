...
var ignoreExt = ['.meta','.pac','.ts']; //忽略文件类型
var classes = {};
var isError = false;
...
//扫描文件
function scanResFiles(bundle,dir) {
        ...
        if (stat.isDirectory()) {
            ...
            if (!isIgnore(file_path)) {
                ...
                let has = classes[classname][filebasename].find(function(element){
                    ...
                    let elementPath = element.path.replace(bundle,'');
                    let itemPath = item.path.replace(bundle,'');
                    return elementPath == itemPath;
                });
                if(has){
                    // isError = true;
                    Editor.warn(`auto gen r file: 相同文件夹下存在 ${file_path} 文件名重复`);
                    continue;
                }
                
                // Editor.log(`item  ${JSON.stringify(item)}`);
                item.bundle = `${ Editor.assetdb.fspathToUrl(`${bundle}`)}/`;
...
function genSceneString(){
    let fspath = Editor.assetdb.urlToFspath(assets_url);
    let scenes = {};
...
function genBundleString(bundles){
    Editor.log('auto gen R file:genBundleString begin gen R file');
    let content = "\n  //Bundle 开始\n";
    content += `  export let bundle = {\n`;
    for (let i = 0; i < bundles.length; i++) {
        const bundle = bundles[i];
        let _arr = bundle.split('\\');
...
function genRun() {
    ...
    let fspath = Editor.assetdb.urlToFspath(assets_url);
    let bundles = utils.seachBundle(fspath);
    // Editor.log(`${bundles.length}`)
    for (let i = 0; i < bundles.length; i++) {
        const bundle = bundles[i];
        scanResFiles(bundle,bundle);
    }
    //扫描所有文件夹
    ...
    for (const classname in classes) {
        ...
        for (const variable in _class) {
            ...
            if(arr.length == 1){
                ...
                dburl = dburl.replace(item.bundle, "");
                ...
                for (let i = 0; i < arr.length; i++) {
                    ...
                    dburl1 = dburl1.replace(element.bundle, "");
    ...
    // Bundle
    content += genBundleString(bundles);
...
function checkGenR(arg, operation) {
    ...
    if (arg && arg.length > 0) {
        ...
        if(pathOrUrl.includes('.ts') && operation == OPERATION_MOVE){
            checkTS(pathOrUrl);
        }
        //自动生成R文件
...
module.exports = {
    ...
    messages: {
        ...
        'search-bundle'(){
            Editor.log("auto gen R file: search-bundle");
            let fspath = Editor.assetdb.urlToFspath(assets_url);
            let bundles = utils.seachBundle(fspath);
